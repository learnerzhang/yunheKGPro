...
if [ "$cmd" = "kgstart" ]; then
    ...
    var=$(ps -ef | grep "cron_task" | grep -v grep | wc -l)
    if [ 0 != $var ]; then
        ps -ef|grep "cron_task" |grep -v grep|cut -c 9-16| xargs kill -9
        sleep 2
        if [ $? == 0 ];then
            echo "kill cron_task success..."
        else
            echo "kill cron_task fail"
        fi
    fi

    nohup python cron_task.py>task.log 2>&1 &
    echo "start cron_task ..."
fi 